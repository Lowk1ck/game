...
function  createtre() {
    let thre = document.createElement('div')
    thre.classList.add('thre')
    let size = `${random(20,100)}`
    let coor = gameBox.getBoundingClientRect()
    let x = random(0, coor.width - size)
    let y = random(0, coor.height - size)

    thre.style.width = size + 'px' 
    thre.style.height = size + 'px'
    thre.style.background = `rgb(${random(0,255)},${random(0,255)},${random(0,255)})`
    thre.style.top = y + 'px'
    thre.style.left = x + 'px'
